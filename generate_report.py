...
Today's date: {today}
MISSION: Use the web_search tool to find TODAY'S actual latest news for each topic, then create a single, cohesive report of approximately {TARGET_WORDS} words. Search at least 6-8 times across different topics to gather real, current information before writing.
...
- Current real-world developments as of today
...
- Reference real organizations, people, and events found in your web searches
...
def save_pdf(md_path: Path, word_count: int) -> Path:
    ...
    html_body = md_lib.markdown(raw_md, extensions=["extra", "tables", "toc", "nl2br"])
    css = CSS(string="""
        @page {
            size: A4;
        ...
        body { font-family: 'Georgia', 'Times New Roman', serif; font-size: 11pt; line-height: 1.75; color: #1a1a1a; }
        h1 { font-size: 20pt; font-weight: bold; color: #111; margin-top: 0; margin-bottom: 4pt; line-height: 1.3; border-bottom: 2.5pt solid #1a1a1a; padding-bottom: 6pt; }
        h2 { font-size: 14pt; font-weight: bold; color: #1a1a1a; margin-top: 20pt; margin-bottom: 6pt; border-left: 4pt solid #2563eb; padding-left: 8pt; }
        h3 { font-size: 11.5pt; font-weight: bold; color: #333; margin-top: 12pt; margin-bottom: 4pt; }
        p { margin: 0 0 9pt 0; text-align: justify; hyphens: auto; }
        em { font-style: italic; color: #444; }
        strong { font-weight: bold; }
        hr { border: none; border-top: 1pt solid #ddd; margin: 14pt 0; }
        blockquote { border-left: 3pt solid #2563eb; margin: 10pt 0; padding: 4pt 12pt; background: #f0f4ff; color: #333; }
        code { font-family: 'Courier New', monospace; font-size: 9.5pt; background: #f5f5f5; padding: 1pt 3pt; }
        table { width: 100%; border-collapse: collapse; margin: 10pt 0; font-size: 10pt; }
        th { background: #1a1a1a; color: white; padding: 5pt 8pt; text-align: left; }
        td { border-bottom: 0.5pt solid #ddd; padding: 4pt 8pt; }
    """)
    html_doc = f"""<!DOCTYPE html>
<html lang="en"><head><meta charset="utf-8"><title>Daily English Report — {date_str}</title></head>
<body>
<div style="background:#f0f4ff;border:1pt solid #2563eb;border-radius:4pt;padding:6pt 10pt;margin:0 0 16pt 0;font-size:10pt;color:#1e3a8a;font-family:Georgia,serif;">
📊 <strong>Today's Stats:</strong> {word_count:,} words &nbsp;|&nbsp; {reading_time} min at 120 WPM &nbsp;|&nbsp; {date_str}
</div>
{html_body}
</body></html>"""
...
def send_email(pdf_path: Path, word_count: int) -> bool:
    ...
    with open(pdf_path, "rb") as f:
        ...
        attachment.add_header("Content-Disposition", "attachment", filename=f"english-report-{date_str}.pdf")
...
def _callout(text: str, emoji: str = "📊") -> dict:
    return {"object": "block", "type": "callout",
            "callout": {"icon": {"type": "emoji", "emoji": emoji}, "rich_text": _parse_inline(text)}}
def markdown_to_notion_blocks(md_text: str, word_count: int, date_str: str) -> list[dict]:
    now = datetime.now(JST)
    reading_time = round(word_count / 120)
    blocks: list[dict] = [
        _callout(f"📊 {word_count:,} words  |  {reading_time} min at 120 WPM  |  {date_str}", "📖"),
    ]
...
def send_to_notion(md_path: Path, word_count: int) -> bool:
    ...
    response = notion.pages.create(
        ...
        properties={
            "title": {
                "title": [{"text": {"content": f"Daily English Report — {date_str} ({weekday})"}}]
            }
        },
        children=blocks[:100],
    )
